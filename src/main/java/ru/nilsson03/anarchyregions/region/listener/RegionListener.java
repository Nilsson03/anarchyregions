...
public class RegionListener implements Listener {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        // Проверяем по координатам (location.equals() не работает после разрушения
        // блока)
    ...
    public void interactWithRegion(PlayerInteractEvent event) {
        ...
        if (clickedBlock == null || clickedBlock.getType() == Material.AIR) {
            ConsoleLogger.debug("anarchyregions",
                    "Player %s tried to interact with region (clicked block is null or air)", player.getName());
            return;
        }
        ...
        if (regionManager.regionExistsInLocation(location)) {
            ...
            ConsoleLogger.debug("anarchyregions", "Location: %s, Core Location: %s", location.toString(), coreLocation.toString());

            if (location.equals(coreLocation) && event.getAction() == Action.RIGHT_CLICK_BLOCK) {
                if (regionManager.isOwner(region, player.getUniqueId())) {
                    ConsoleLogger.debug("anarchyregions", "Player %s is owner of region at %s, opening menu",
                            player.getName(), formatLocation(location));
                    RegionMenu.openMenu(player, region);
                } else {
                    ConsoleLogger.debug("anarchyregions", "Player %s is not owner of region at %s, cancelling event",
                            player.getName(), formatLocation(location));
                    event.setCancelled(true);
                    player.sendMessage(messagesConfig.getString("messages.region_not_owner"));
                }
