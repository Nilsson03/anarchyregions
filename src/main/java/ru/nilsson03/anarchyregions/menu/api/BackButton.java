...
public class BackButton extends PageItem {
    ...
    public ItemProvider getItemProvider(PagedGui<?> gui) {
        ...
        String type = parameterFile.getValueAs("inventories.buttons.back-button.type", String.class);
        String displayName = UniversalTextApi.colorize(parameterFile.getValueAs("inventories.buttons.back-button.name", String.class));
        List<String> lore = UniversalTextApi.colorize(parameterFile.getValueAs("inventories.buttons.back-button.lore", List.class));

        ItemStack itemStack;
        if (type.equalsIgnoreCase("head")) {
            String url = parameterFile.getValueAs("inventories.buttons.back-button.head-id", String.class);
            itemStack = ItemUtil.createHead(url)
                    .setDisplayName(displayName)
                    .setLore(lore)
                    .build();
        } else {
            String materialName = parameterFile.getValueAs("inventories.buttons.back-button.material", String.class);
